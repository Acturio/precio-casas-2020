...
casas %>% ggplot(aes(x = reorder(Neighborhood, SalePrice_m2, median), y = SalePrice_m2)) +
  geom_boxplot() +
  geom_jitter()
...
casas_rec <- recipe(SalePrice_m2 ~ . , data = casas_train) %>%
  ...
  step_mutate(Exter_Cond = fct_collapse(Exter_Cond, Bad = c("Po", "Fa"),
              ...
              Neighborhood = fct_collapse(Neighborhood, Greens_GrnHill = c("Greens", "GrnHill"),#
                                          Blueste_Veenker = c("Blueste", "Veenker"),#
                                          Blmngtn_Timber = c("Blmngtn", "Timber"),#
                                          NPkVill_NAmes = c("NPkVill", "NAmes"),#
                                          ClearCr_NridgHt_StoneBr = c("ClearCr","NridgHt", "StoneBr"),#
                                          BrDale_OldTown = c("BrDale", "OldTown"), #
                                          SWISU_Sawyer = c("SWISU", "MeadowV, IDOTRR")#
                                          ),
...
write_csv(submission1_KAGGLE, "Acturio_sub2.csv")  
